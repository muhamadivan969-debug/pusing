...
function computeProgress(s: SignalRow): { pct: number; color: string } {
  ...
  if (direction === 'BUY') {
    ...
    if (current_price >= entry_price) {
      const pct =
        ((current_price - entry_price) / (tp1 - entry_price)) * 100
    ...
    const pct =
      ((entry_price - current_price) / (entry_price - stop_loss)) * 100
  ...
  if (current_price <= entry_price) {
    const pct =
      ((entry_price - current_price) / (entry_price - tp1)) * 100
  ...
  const pct =
    ((current_price - entry_price) / (stop_loss - entry_price)) * 100
...
export default function SignalPage() {
        ...
        if (active && saved) {
          const ids = new Set(
            saved
              .map(
                (row) =>
                  (row.signal_snapshot as { id?: string })?.id
              )
  ...
  // Sama seperti alur unlock di Detail Saham (dokumen 6.5):
  // 1 token per saham unik per hari.
  // RPC ini yang menentukan angka, bukan client.
  ...
  function handleCopy(s: SignalRow) {
    const text = [
      ...
      s.buy_area_low != null && s.buy_area_high != null
        ? `Buy Area: ${formatHarga(s.buy_area_low)} - ${formatHarga(
            s.buy_area_high
          )}`
        : null,

      s.tp1 != null
        ? `TP1: ${formatHarga(s.tp1)}`
        : null,

      s.tp2 != null
        ? `TP2: ${formatHarga(s.tp2)}`
        : null,

      s.stop_loss != null
        ? `Stop Loss: ${formatHarga(s.stop_loss)}`
        : null,

      s.support_level != null
        ? `Support: ${formatHarga(s.support_level)}`
        : null,

      s.resistance_level != null
        ? `Resistance: ${formatHarga(s.resistance_level)}`
        : null,
                ...
                >
                  ...
                  {s.unlocked ? (
                    ...
                    <p className="text-slate-500 text-xs mt-2">
                      Buy Area, TP, SL, Support & Resistance
                      terkunci
                    </p>
                  )}
                </Link>
                ...
                {!s.unlocked && (
                  <div className="mt-2 space-y-1.5">
                    ...
                    >
                      {unlockingId === s.id
                        ? 'Memproses...'
                        : 'Lihat Penjelasan Lengkap (1 Token)'}
                    </button>
                    ...
                    {unlockErr &&
                      unlockErr.id === s.id && (
                        <p className="text-[#EF4444] text-[10px] text-center">
                          {unlockErr.message}
                        </p>
                      )}
                  </div>
