  ...
  switch (section) {
      ...
      return (
        <div className="space-y-2 text-sm text-slate-300 leading-relaxed">
          <p>
            ...
            Target, dan Stop Loss dihasilkan dari pembacaan struktur harga (confluence
            multi-timeframe: support, resistance, buy area, TP1/TP2, stop loss) oleh sistem.
            Ini <strong>bukan jaminan profit</strong> dan bukan rekomendasi investasi resmi.
          </p>
          <p>
            Perdagangan saham memiliki risiko kerugian. Selalu lakukan riset mandiri dan
    ...
    case 'cara-kerja-ai':
      return (
        <div className="space-y-2 text-sm text-slate-300 leading-relaxed">
          <p>
            Angka pada Kartu Sinyal (Buy Area, TP1, TP2, Stop Loss, Support, Resistance)
            <strong> selalu dihitung oleh sistem/engine</strong> berdasarkan indikator
            teknikal dan struktur harga (confluence multi-timeframe) — bukan dikarang oleh AI.
            Sistem tidak menampilkan Win Rate, Confidence Score, atau klaim statistik lain
      ...
      <div className="rounded-xl bg-white/5 border border-white/10 divide-y divide-white/5 overflow-hidden">
        {SECTIONS.map((s) => (
          <div key={s.key}>
            <button
              onClick={() =>
                setOpen(open === s.key ? ('' as SectionKey) : s.key)
              }
