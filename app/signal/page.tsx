...
export default function SignalPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!active) return
      setSignals((data as unknown as SignalRow[]) ?? [])
      setLoading(false)
    }
  ...
  const filtered = useMemo(() => {
    ...
    if (query) {
      ...
      list = list.filter(
        (s) =>
          s.stocks?.ticker.includes(q) || s.stocks?.name.toUpperCase().includes(q)
      )
    }
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="min-w-0">
                    <p className="font-semibold text-sm">{s.stocks?.ticker ?? '-'}</p>
